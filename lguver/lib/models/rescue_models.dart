...
class SOSRequest {
  ...
  List<String> backupUnitIds;
  /// When set, this SOS is in history (completed/resolved).
  final DateTime? completedAt;
  /// Close reason when cancelled (e.g. false_alarm). Not used to screen incoming SOS.
  final String? closeReason;
  /// Reverse-geocoded street address from GPS (optional).
  String? address;
  ...
  List<String> get allAssignedUnitIds {
    ...
    if (assignedUnitId != null && assignedUnitId!.isNotEmpty) {
      ids.add(assignedUnitId!);
    }
    for (final id in backupUnitIds) {
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (backupUnitIds.isNotEmpty) 'backupUnitIds': backupUnitIds,
  ...
  factory SOSRequest.fromJson(Map<String, dynamic> json) {
    ...
    final backupRaw = json['backupUnitIds'];
    final backupUnitIds = <String>[];
    if (backupRaw is List) {
      for (final e in backupRaw) {
        final id = e?.toString() ?? '';
        if (id.isNotEmpty) backupUnitIds.add(id);
      }
    }
    final closeReason = json['closeReason'] as String?;
