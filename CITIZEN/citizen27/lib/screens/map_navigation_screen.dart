...
class _MapNavigationScreenState extends State<MapNavigationScreen> {
  ...
  DateTime? _lastCitizenPosUpdateAt;
  ...
  /// Checks if rescuer has deviated from route; reroutes if so. Runs until rescue completed/cancelled.
  void _startRerouteChecker() {
    _rerouteTimer?.cancel();
    _rerouteTimer = Timer.periodic(const Duration(seconds: 4), (_) async {
      ...
      // Very close to SOS: OSRM often returns unstable geometry; only reroute when
      // the citizen's pin moves (not for tiny GPS jitter).
      if (distToTargetKm < 0.08) {
        if (targetMovedKm <= 0.025) {
          return;
        }
      } else if (distToTargetKm < 0.05) {
        if (targetMovedKm <= 0.03) {
          return;
        }
      }
      ...
      final shouldReroute = route.length < 2 ||
          ...
          targetMovedKm > 0.03; // SOS moved more than ~30m since last reroute
  ...
  Future<void> _syncDispatchProgress(RescueProvider provider) async {
    ...
    final etaMinutes = provider.osrmEtaMinutes > 0
        ? provider.osrmEtaMinutes.round().clamp(0, 9999)
        : null;
  ...
  void _watchCitizenLocation() {
    ...
    _citizenLocationSub = provider.firebaseSync
      ...
      setState(() {
        ...
        _lastCitizenPosUpdateAt = DateTime.now();
      });
    });
  }
        ...
        children: [
          Consumer<MapThemeProvider>(
            builder: (context, mapTheme, _) {
              ...
              final driverPos = provider.currentPosition;
              if (driverPos != null) _checkArrival(driverPos);

              // Ensure the UI banner appears even if distance computation lags a bit
              // (source of truth becomes the unit status from Firebase).
              final unit = provider.rescueUnits
                  .where((u) => u.id == widget.responderUnit.id)
                  .firstOrNull;
              final unitStatus = unit?.status ?? widget.responderUnit.status;
              // Don't trust a stale Firebase "onScene" too early.
              // Only mark arrived when we are actually close to the latest SOS location.
              if (unitStatus == UnitStatus.onScene && !_arrived) {
                if (driverPos != null &&
                    _citizenLivePosition != null &&
                    _lastCitizenPosUpdateAt != null) {
                  final distKm =
                      GeoUtils.haversineKm(driverPos, _citizenLivePosition!);
                  final fresh = DateTime.now()
                          .difference(_lastCitizenPosUpdateAt!)
                          .inSeconds <=
                      20;
                  if (fresh && distKm < 0.05) {
                ...
                children: [
                  ...
                  MarkerLayer(
                    markers: [
                      ...
                      ..._medicalFacilities.map(
                          (f) => Marker(
                            point: f.position,
                            width: 28,
                            height: 28,
                            child: GestureDetector(
                              onTap: () => _routeToFacility(f),
                              child: Container(
                                decoration: BoxDecoration(
                                  color: _selectedFacility?.id == f.id
                                      ? (f.layerType == MapLayerType.threeSCenters
                                          ? Colors.deepPurple.shade700
                                          : Colors.red.shade700)
                                      : Colors.white,
                                  shape: BoxShape.circle,
                                  border: Border.all(
                                    color: f.layerType == MapLayerType.threeSCenters
                                        ? Colors.deepPurple.shade700
                                        : Colors.red.shade700,
                                    width: 2,
                                  ),
              ...
              builder: (context, provider, _) {
                ...
                return AnimatedContainer(
                  ...
                  child: FloatingActionButton.extended(
                    ...
                    onPressed:
                        showArrive ? _markArrivedManually : _confirmCancelResponse,
                    backgroundColor:
  ...
  Color _uiSecondaryColor(RescueMapStyle style) {
    return _isDarkMapStyle(style) ? Colors.white70 : const Color(0xFF294560);
  }
    ...
    return Positioned(
      ...
      child: Consumer2<RescueProvider, MapThemeProvider>(
        builder: (context, provider, mapTheme, _) {
          final primary = _uiPrimaryColor(mapTheme.style);
          final secondary = _uiSecondaryColor(mapTheme.style);
          final shadow = _uiShadowColor(mapTheme.style);
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await context.read<RescueProvider>().firebaseSync.updateDispatchProgress(
                        widget.sosRequest.id,
                        status: 'completed',
                        phase: 'completed',
                      );
                  await context.read<RescueProvider>().completeSOS(widget.sosRequest);
